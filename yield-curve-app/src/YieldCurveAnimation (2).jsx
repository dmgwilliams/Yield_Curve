...
// Market data fetched from API at runtime
...
export default function YieldCurveAnimation() {
  ...
  // Fetch market data (SPY, QQQ, BTC, ETH) aligned to yield curve dates
  useEffect(() => {
    let cancelled = false;

    function findPrice(dailyMap, dateStr) {
      if (!dailyMap) return null;
      if (dailyMap[dateStr] !== undefined) return dailyMap[dateStr];
      const d = new Date(dateStr + "T00:00:00Z");
      for (let delta = 1; delta <= 5; delta++) {
        for (const dir of [-1, 1]) {
          const nd = new Date(d);
          nd.setUTCDate(nd.getUTCDate() + dir * delta);
          const k = nd.toISOString().slice(0, 10);
          if (dailyMap[k] !== undefined) return dailyMap[k];
        }
      }
      return null;
    }
    ...
    async function fetchMarket() {
      try {
        ...
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const json = await res.json();
        if (cancelled || !json.success || !json.prices) return;

        const p = json.prices;
        const built = {};

        for (const { date } of yieldData) {
          const entry = {};
          let hasAny = false;
          for (const key of ["spy", "qqq", "btc", "eth"]) {
            const val = findPrice(p[key], date);
            if (val !== null) { entry[key] = val; hasAny = true; }
          }
          if (!hasAny) continue;
          ...
          fwd.setUTCMonth(fwd.getUTCMonth() + 3);
          const fwdStr = fwd.toISOString().slice(0, 10);
          for (const key of ["spy", "qqq", "btc", "eth"]) {
            const fv = findPrice(p[key], fwdStr);
            if (fv !== null) entry[key + "_fwd"] = fv;
          }
        ...
        console.log("Market data unavailable:", err.message);
      } finally {
        if (!cancelled) setMarketLoading(false);
      }
    }

    if (yieldData.length > 0) fetchMarket();
    return () => { cancelled = true; };
  }, [yieldData]);
  ...
  // Market data lookup
  const mkt = marketData[currentData.date] || {};
